...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateDisplayName(name: String): Result<Unit> {
        ...
        return try {
            val mappingDoc = firestore.collection("uid_mapping").document(uid).get().await()
            val uniqueId = mappingDoc.getString("uniqueId") ?: return Result.failure(Exception("User not found"))
            usersCollection.document(uniqueId).update("displayName", name).await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
